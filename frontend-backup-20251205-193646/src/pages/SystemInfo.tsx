...
export default function SystemInfo() {
  ...
  const getPerformanceStatus = (percentage: number) => {
    if (percentage >= 90) return { color: 'red', label: 'Critical', icon: AlertCircle };
    if (percentage >= 75) return { color: 'yellow', label: 'Warning', icon: AlertCircle };
          ...
          {metrics && (
            ...
            >
              ...
              <motion.div
                ...
                whileHover={{ scale: 1.02, y: -5 }}
                className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-xl p-4 sm:p-6 border border-gray-200 dark:border-gray-700 hover:shadow-2xl hover:shadow-purple-500/20 transition-all duration-300 group relative overflow-hidden"
              >
                ...
                <div className="relative z-10">
                  <div className="flex items-center justify-between mb-4">
                    <motion.div 
                      whileHover={{ rotate: 360 }}
                      transition={{ duration: 0.6 }}
                      className="p-3 bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl shadow-lg transform group-hover:scale-110 transition-all duration-300"
                    >
                      <MemoryStick className="w-6 h-6 text-white" />
                    </motion.div>
                    <div className="text-right">
                      <motion.div 
                        key={metrics.memory.percentage}
                        initial={{ scale: 1.2, opacity: 0 }}
                        animate={{ scale: 1, opacity: 1 }}
                      ...
                      <div className="text-xs text-gray-500 flex items-center gap-1 justify-end">
                        {React.createElement(getPerformanceStatus(metrics.memory.percentage).icon, { className: 'w-3 h-3' })}
                        {getPerformanceStatus(metrics.memory.percentage).label}
                      </div>
                    </div>
                  </div>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-medium text-gray-900 dark:text-white px-2 py-0.5 bg-gray-100 dark:bg-gray-700 rounded">{formatBytes(metrics.memory.used)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600 dark:text-gray-400 flex items-center gap-1">
                      ...
                      <span className="font-medium text-gray-900 dark:text-white px-2 py-0.5 bg-gray-100 dark:bg-gray-700 rounded">{formatBytes(metrics.memory.total)}</span>
                    </div>
                  </div>
                  <div className="mt-3 h-16 relative">
                    <svg className="w-full h-full" viewBox="0 0 100 40" preserveAspectRatio="none">
                      ...
                      <line x1="0" y1="13.75" x2="100" y2="13.75" stroke="#ef4444" strokeWidth="0.5" strokeDasharray="2,2" opacity="0.4" />
                      <text x="98" y="13" fontSize="2.5" fill="#ef4444" textAnchor="end" opacity="0.6">75%</text>
                      <path d={generateAreaPath(history.memory)} fill="url(#memGradient)" className="opacity-60" />
                      <path d={generatePath(history.memory)} fill="none" stroke="#a855f7" strokeWidth="2" filter="url(#memGlow)" className="drop-shadow-lg" />
                    </svg>
                  </div>
                </div>
              </motion.div>
