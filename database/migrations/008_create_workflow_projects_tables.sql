-- Migration: Create workflow projects and workflow tracking tables
...
DROP VIEW IF EXISTS workflow_project_summary CASCADE;
DROP TABLE IF EXISTS project_workflow_steps CASCADE;
DROP TABLE IF EXISTS workflow_projects CASCADE;

-- Create workflow_projects table (separate from existing projects table)
...
-- Create project_workflow_steps table
...
CREATE INDEX idx_workflow_projects_status ON workflow_projects(status);
CREATE INDEX idx_workflow_projects_created_date ON workflow_projects(created_date);
CREATE INDEX idx_project_workflow_steps_project_id ON project_workflow_steps(project_id);
CREATE INDEX idx_project_workflow_steps_status ON project_workflow_steps(status);
CREATE INDEX idx_project_workflow_steps_assignee ON project_workflow_steps(assignee);

-- Insert sample projects
INSERT INTO workflow_projects (id, name, description, created_date, target_date, status, progress, created_by)
VALUES 
...
INSERT INTO project_workflow_steps (project_id, step_id, step_number, title, description, status, owner_team, assignee, completed_date, route)
VALUES
...
CREATE TRIGGER update_workflow_projects_updated_at BEFORE UPDATE ON workflow_projects
    FOR EACH ROW EXECUTE FUNCTION update_workflow_updated_at_column();

CREATE TRIGGER update_project_workflow_steps_updated_at BEFORE UPDATE ON project_workflow_steps
    FOR EACH ROW EXECUTE FUNCTION update_workflow_updated_at_column();
...
LEFT JOIN project_workflow_steps ws ON p.id = ws.project_id
GROUP BY p.id, p.name, p.description, p.created_date, p.target_date, p.status, p.progress, p.created_by;

COMMENT ON TABLE workflow_projects IS 'Workflow projects table tracking all enterprise projects with EA->SA->CMDB->PM->SE->Agent flow';
COMMENT ON TABLE project_workflow_steps IS 'Workflow steps for each project tracking progress through each phase';
COMMENT ON VIEW workflow_project_summary IS 'Summary view showing project statistics and workflow progress';
